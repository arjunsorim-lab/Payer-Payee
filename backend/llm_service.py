...
PROMPT_VERSION = "provider-groq-money-v3.0"
_ANALYSIS_CACHE = {}
_CHAT_CACHE = {}
...
def _chat_fallback(case_input, question, reason=None):
    ...
    financial = case_input.get("provider_financial_metrics", {})
    ...
    if "allowed" in normalized:
        ...
        answer = f"Potential provider revenue at risk is ${financial.get('potential_revenue_at_risk', 0):,.2f}. The backend calculates it as predicted adjustment plus expected denial exposure; it is exposure, not guaranteed recoverable revenue."
        fields = ["potential_revenue_at_risk", "predicted_adjustment", "expected_denial_exposure"]
