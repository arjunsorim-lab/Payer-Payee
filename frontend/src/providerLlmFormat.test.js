...
test('provider forecast and payer savings are clearly separated and centered', () => {
  ...
  assert.match(app, /Other bills in the \{historyWindowDays\} days before this visit/)
  ...
  assert.match(app, /Possible money-saving check/)
  assert.match(app, /A doctor and billing reviewer must check this/)
  assert.match(app, /scenario\.value_based_case/)
  assert.match(app, /Possible money to save \(needs checking\)/)
  assert.match(app, /not money we have definitely saved/)
