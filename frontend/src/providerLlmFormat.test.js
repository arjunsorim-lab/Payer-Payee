...
test('provider forecast shows one claim-anchored billed comparison', () => {
  ...
  assert.doesNotMatch(providerView, /We could not find a fair bill to compare/)
  assert.match(app, /scenario\.payer_savings_prediction/)
  assert.match(app, /function PeerComparisonPredictionSummary/)
  assert.match(app, /Claim-anchored billed comparison/)
  assert.match(app, /Billed amount comparison for this visit/)
  assert.match(app, /prediction\.possible_billed_difference/)
  assert.match(app, /Recorded Charge_Amount on claim/)
  assert.match(app, /Exact source rows used in the calculation/)
  assert.match(app, /Billed amount used/)
  ...
  assert.match(app, /Why this claim was chosen instead of the other claims/)
  assert.match(app, /selectionAudit\.plain_language_steps/)
  assert.match(app, /selectionAudit\.selection_guardrail/)
  assert.match(app, /selectionAudit\.episode_candidates/)
  assert.match(app, /selectionAudit\.claim_candidates/)
  assert.match(app, /The price did not choose the peer/)
  assert.match(app, /Use the billed difference only as a review flag/)
  assert.match(app, /View full episode ranking/)
  assert.match(app, /View full claim ranking/)
  assert.doesNotMatch(providerView, /<PayerProcedureComparison/)
  assert.doesNotMatch(providerView, /<ValueBasedRectificationCase/)
  assert.doesNotMatch(providerView, /Patient-specific comparison/)
  assert.doesNotMatch(providerView, /<CrossPatientComparatorContent/)
...
test('All Encounters features source-marked reference claims without a fixed identity', () => {
  assert.match(app, /const isReferenceClaim = \(claim\)/)
  assert.match(app, /claim\.workbookFields\?\.Reference_Claim_Flag/)
  assert.match(app, /isReferenceClaim\(left\)/)
  assert.doesNotMatch(app, /FEATURED_DEMO_CLAIM_ID|['"]CLM\d{5,}['"]/)
